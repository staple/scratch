...
object TFOCS_optimized extends Logging {

  def minimize(
    f: SmoothFunction[RDD[Double]],
    A: LinearFunction[Vector, RDD[Double]],
    h: ProxCapableFunction[Vector],
    data: RDD[Vector],
    features: Int,
    x0: Vector,
    numIterations: Int = 500,
    convergenceTol: Double = 1e-13)(
      implicit vx: VectorSpace[Vector],
      vy: VectorSpace[RDD[Double]]): (Vector, Array[Double]) = {
    ...
    //   val bcX = data.context.broadcast(x)

    //   // Sum the loss function and gradient computed for each training example.
    //   val (loss, grad) = data.treeAggregate((0.0, Vectors.zeros(x.size), 0L))(
    //     seqOp = (c, v) => (c, v) match { case ((loss, grad), (label, features)) =>
      ...
      for (nIter <- 1 to numIterations) {
        ...
        breakable {
          while (true) {
            ...
            var g_Ay: Option[RDD[Double]] = None
            if (backtrack_simple) {
              // println("one")
              val (f_y_, g_y_) = f.two(a_y, data, features) // Spark job
              f_y = f_y_
              g_y = g_y_
            } else {
              // println("two")
              val Value(None, Some(g_Ay_)) = f(a_y, Mode(false, true))
              // f_y is not necessary.
              vy.cache(g_Ay_)
              g_Ay = Some(g_Ay_)
              g_y = A.t(g_Ay_) // Spark job      
            }
            ...
            if (backtrack_simple) {
              ...
              localL = 2.0 * vy.dot(vy.combine(1.0, a_x, -1.0, a_y), vy.combine(1.0, g_Ax, -1.0, g_Ay.get)) / xy_sq // Spark job
            }
