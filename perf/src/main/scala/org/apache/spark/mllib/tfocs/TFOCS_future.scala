      ...
      for (nIter <- 1 to numIterations) {
        ...
        breakable {
          while (true) {
            ...
            val (f_y_, g_Ay) = f.future(a_y) // Future spark job
            ...
            if (backtrack_simple) {
              ...
              val f_y = Await.result(f_y_, Duration.Inf)
